...
/**
 * Local AES-256-GCM keystore for API keys and similar secrets.
 *
 * Layout on disk:
 *   <runtime-services-data>/secrets.enc      — JSON map { id → encrypted envelope }
 *   <runtime-services-data>/.keystore.salt   — 32 random bytes, generated once
 *
 ...
 * threat needs a real OS keychain.
 */
...
async function writeStore(store: StoreFile, storePaths: KeystorePaths = paths): Promise<void> {
  await mkdir(dirname(storePaths.secretsFile), { recursive: true });
  const tmp = `${storePaths.secretsFile}.tmp-${process.pid}`;
  await writeFile(tmp, `${JSON.stringify(store, null, 2)}\n`, 'utf8');
  await chmod(tmp, 0o600);
  await rename(tmp, storePaths.secretsFile);
}
...
async function loadOrCreateSalt(storePaths: KeystorePaths = paths): Promise<Buffer> {
  ...
  await mkdir(dirname(storePaths.keystoreSaltFile), { recursive: true });
  const tmp = `${storePaths.keystoreSaltFile}.tmp-${process.pid}`;
  await writeFile(tmp, salt);
  await chmod(tmp, 0o600);
  await rename(tmp, storePaths.keystoreSaltFile);
  return salt;
}
 ...
 * propagate. */
export async function getSecret(id: string, storePaths: KeystorePaths = paths): Promise<string | undefined> {
...
export async function setSecret(id: string, plaintext: string, storePaths: KeystorePaths = paths): Promise<void> {
  const key = await deriveKey(storePaths);
...
export async function removeSecret(id: string, storePaths: KeystorePaths = paths): Promise<boolean> {
  const store = await readStore(storePaths);
